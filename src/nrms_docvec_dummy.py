...
his_input_title = np.array([np.random.rand(*his_input_title_shape) for _ in range(BATCH_SIZE)])
# Generate some random input data for input_2
pred_input_title = np.array([np.random.rand(*pred_input_title_shape) for _ in range(BATCH_SIZE)])
